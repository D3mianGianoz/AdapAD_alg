...
class NormalDataPredictor:
    def __init__(self, lstm_layer, lstm_unit, lookback_len, prediction_len):
        ...
        self.predictor = LSTMPredictor(
            prediction_len, lookback_len, hidden_size, num_layers, lookback_len
        )
    ...
    def update(self, epoch_update, lr_update, past_observations, recent_observation):
        ...
        for epoch in range(num_epochs):
            ...
            loss = criterion(
                predicted_val,
                torch.from_numpy(np.array(recent_observation).reshape(1, -1)).float(),
            )
            loss.backward(retain_graph=True)
            optimizer.step()
...
class AnomalousThresholdGenerator:
    ...
    def update(self, epoch_update, lr_update, past_errors, recent_error):
        ...
        for epoch in range(num_epochs):
            ...
            loss = criterion(
                predicted_val,
                torch.from_numpy(np.array(recent_error).reshape(1, -1)).float(),
            )
            loss.backward(retain_graph=True)
            optimizer.step()
            # for early stopping
